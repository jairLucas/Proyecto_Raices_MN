const puntoFijo = async (params, puntoInicial, g) => {
    const {
        funcion,
        precision,
        intervalo,
    ...
    console.log("Ejecutando metodo de punto fijo");
    ...
    if (g === null) {
        callbackError("Error. La función g(xn) ingresada es inválida.");
        return;
    }
...
app.component("metodo-punto-fijo", {
    template: `
        <div>
        <form @submit.prevent :style="estilos['contenedor-form']">
            <label for="valor-min">g(x<sub>n</sub>): </label>
    ...
    setup(props) {
        ...
        const calcular = () => {
            ...
            props.solver.run(puntoFijo, puntoInicial.value, funcionDerivada.value);
        };
